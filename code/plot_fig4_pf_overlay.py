...
def latest_obj(problem: str, nis: int, run: int = 20) -> Path:
    run_dir = base / problem / f"nis_{nis:03d}" / f"run_{run:03d}"
...
def plot_group(group_key: str, title_prefix: str, label_prefix: str):
    ...
    for ax, problem in zip(axes, problems):
        for nis in nis_values:
            arr = load_obj(latest_obj(problem, nis, 20))
            if arr.size == 0:
                continue
            sc = ax.scatter(arr[:, 0], arr[:, 1], s=9, alpha=0.65, c=colors[nis], edgecolors='none', label=f'{label_prefix}={nis}')
            if problem == problems[0]:
                handles.append(sc)
                labels.append(f'{label_prefix}={nis}')
    ...
    png = out / f'fig4_{group_key}_pf_overlay_run020_{label_prefix.lower()}_labeled.png'
    svg = out / f'fig4_{group_key}_pf_overlay_run020_{label_prefix.lower()}_labeled.svg'
...
plot_group('uf1237', 'UF1/2/3/7', 'NIS')
plot_group('lsmop1459', 'LSMOP1/4/5/9', 'NIS')

...
% stagnation-triggered elite exchange.
    ...
    cfg = applyECMADEWorkspaceConfig(struct( ...
        'subpops',subpops,'archiveSize',archiveSize,'theta',theta, ...
        'stagnationThreshold',stagnationThreshold,'exploitationAlpha',exploitationAlpha, ...
        'initMuF',initMuF,'initMuCR',initMuCR,'fScale',fScale,'crScale',crScale, ...
        'fMax',fMax,'exchangeMode',exchangeMode,'consensusArchive',consensusArchive, ...
        'consensusBins',consensusBins,'archiveConsWeight',archiveConsWeight, ...
        'bestGuide',bestGuide,'bestConsWeight',bestConsWeight, ...
        'bestCentralWeight',bestCentralWeight,'minSubpopSize',minSubpopSize, ...
        'eliteRatio',eliteRatio,'operatorMode',operatorMode, ...
        'archiveLimitFactor',archiveLimitFactor));
    subpops = cfg.subpops; archiveSize = cfg.archiveSize; theta = cfg.theta;
    stagnationThreshold = cfg.stagnationThreshold; exploitationAlpha = cfg.exploitationAlpha;
    initMuF = cfg.initMuF; initMuCR = cfg.initMuCR; fScale = cfg.fScale; crScale = cfg.crScale;
    fMax = cfg.fMax; exchangeMode = cfg.exchangeMode; consensusArchive = cfg.consensusArchive;
    consensusBins = cfg.consensusBins; archiveConsWeight = cfg.archiveConsWeight;
    bestGuide = cfg.bestGuide; bestConsWeight = cfg.bestConsWeight;
    bestCentralWeight = cfg.bestCentralWeight; minSubpopSize = cfg.minSubpopSize;
    eliteRatio = cfg.eliteRatio; operatorMode = cfg.operatorMode;
    archiveLimitFactor = cfg.archiveLimitFactor;
    ...
    [archiveDec,archiveObj,archiveHits,archiveMasks] = updateParetoArchive( ...
        [],[],[],[],Population,subpopIDs,Global.N,consensusArchive,consensusBins,archiveConsWeight,subpops,archiveLimitFactor);
    ...
    while Global.NotTermination(Population)
        ...
        for sid = 1:subpops
            ...
            for mi = 1:numel(members)
                ...
                mutant = mutateECMADEPythonStyle(i,sid,PopDec,subpopIDs,bestIndex,Fi, ...
                    generation,Global.evaluation,Global.N,exploitationAlpha,operatorMode);
        ...
        [newArchiveDec,newArchiveObj,newArchiveHits,newArchiveMasks,archiveChanged] = updateParetoArchive( ...
            ...
            consensusArchive,consensusBins,archiveConsWeight,subpops,archiveLimitFactor);
        ...
        if stagnationThreshold > 0 && stagnation > stagnationThreshold
            [Population,subpopIDs,muF,muCR,recentF,recentCR] = exchangeInformation( ...
                Population,subpopIDs,muF,muCR,recentF,recentCR,subpops,exchangeMode,eliteRatio);
            stagnation = 0;
...
function cfg = applyECMADEWorkspaceConfig(cfg)
    if evalin('base','exist(''ECMADE_MOO_KB_THETA'',''var'')')
        userCfg = evalin('base','ECMADE_MOO_KB_THETA');
...
function mutant = mutateECMADEPythonStyle(i,sid,PopDec,subpopIDs,bestIndex,F,generation,maxFE,popSize,alpha,operatorMode)
    members = find(subpopIDs == sid);
    best = PopDec(bestIndex,:);
    mode = lower(operatorMode);
    if strcmp(mode,'mixed')
        modes = {'rand2','best2','hybrid'};
        mode = modes{mod(sid-1,numel(modes))+1};
    end
    if strcmp(mode,'rand2')
        r = chooseIndicesFromSubpop(members,i,5);
        mutant = PopDec(r(1),:) + F*(PopDec(r(2),:)-PopDec(r(3),:)) + ...
            F*(PopDec(r(4),:)-PopDec(r(5),:));
    elseif strcmp(mode,'best2')
...
function [archiveDec,archiveObj,archiveHits,archiveMasks,changed] = updateParetoArchive(oldDec,oldObj,oldHits,oldMasks,Population,ids,N,consensusArchive,consensusBins,archiveConsWeight,subpops,archiveLimitFactor)
    if isempty(oldObj)
    ...
    limit = max(N*archiveLimitFactor,N);
...
function [Population,ids,muF,muCR,recentF,recentCR] = exchangeInformation(Population,ids,muF,muCR,recentF,recentCR,subpops,exchangeMode,eliteRatio)
    if strcmpi(exchangeMode,'none')
        return;
    end
    Obj = Population.objs;
    Con = Population.cons;
    ...
    if eliteRatio <= 0
        ...
        eliteCount = max(1,ceil(eliteRatio*numel(Population)));

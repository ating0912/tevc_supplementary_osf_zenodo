% P0-lite GDE3 only: OR-Library port1, K={5,10,15,20,25,30}, 30 runs.
...
if p0LiteSmoke
    outRoot = fullfile(scriptDir,'p0_lite_outputs','smoke_port1_gde3_only');
end
...
P0LiteUtils.writeConfig(outRoot,cfg,numel(mu),'GDE3');

for kk = 1:numel(cfg.KValues)
    K = cfg.KValues(kk);
    fprintf('=== GDE3 K=%d ===\n',K);
    for run = 1:cfg.runs
        fprintf('GDE3 K=%d Run %03d/%03d\n',K,run,cfg.runs);
        runDir = fullfile(outRoot,sprintf('K_%02d',K),'GDE3',sprintf('run_%03d',run));
        ...
        t = tic;
        rng(run,cfg.rngType);
        G = GLOBAL('-algorithm',@GDE3,'-problem',{@PortfolioORLIB,cfg.dataPath,K}, ...

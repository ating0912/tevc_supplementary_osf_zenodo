...
$out = Join-Path $root "nsga2_outputs\deb_c_uf6_10_maxfe10000"
...
try {
    foreach ($problem in 6..10) {
        $name = "UF$problem"
        $objectives = if ($problem -ge 8) { 3 } else { 2 }
        ...
        & $gcc "-DUF_PROBLEM=$problem" "-O2" "-std=gnu89" @sources `
            (Join-Path $src "problemdef_uf.c") "-o" $exe "-lm"
        if ($LASTEXITCODE -ne 0) { throw "Compilation failed for $name" }

        $bounds = for ($d = 1; $d -le 30; $d++) {
            if ($problem -ge 8) {
                if ($d -le 2) { "0 1" } else { "-2 2" }
            }
            else {
                if ($d -eq 1) { "0 1" } else { "-1 1" }
            }
        }
        $inputLines = @("100","100","$objectives","0","30") + $bounds +

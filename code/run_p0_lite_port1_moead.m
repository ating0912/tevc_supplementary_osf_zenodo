% P0-lite MOEA/D only: OR-Library port1, K={5,10,15,20,25,30}, 30 runs.
...
if p0LiteSmoke
    outRoot = fullfile(scriptDir,'p0_lite_outputs','smoke_port1_moead_only');
end
...
P0LiteUtils.writeConfig(outRoot,cfg,numel(mu),'MOEAD');

for kk = 1:numel(cfg.KValues)
    K = cfg.KValues(kk);
    fprintf('=== MOEA/D K=%d ===\n',K);
    for run = 1:cfg.runs
        fprintf('MOEAD K=%d Run %03d/%03d\n',K,run,cfg.runs);
        runDir = fullfile(outRoot,sprintf('K_%02d',K),'MOEAD',sprintf('run_%03d',run));
        ...
        t = tic;
        rng(run,cfg.rngType);
        G = GLOBAL('-algorithm',@MOEAD,'-problem',{@PortfolioORLIB,cfg.dataPath,K}, ...
